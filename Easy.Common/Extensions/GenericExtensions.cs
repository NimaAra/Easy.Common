...
public static class GenericExtensions
{
    ...
    {
        DynamicDictionary dynDic = new();
        foreach (PropertyInfo property in @object!.GetType().GetInstanceProperties(inherit))
        {
    ...
    public static T CloneShallowUsingIl<T>(this T @object)
    {
        ...
        if (!CachedIlShallow.TryGetValue(type, out Delegate? myExec))
        {
            ...
            DynamicMethod dymMethod = new DynamicMethod("DoClone", type, new[] { type }, Assembly.GetExecutingAssembly().ManifestModule, true);
            ConstructorInfo? cInfo = @object!.GetType().GetConstructor(new Type[] { });
