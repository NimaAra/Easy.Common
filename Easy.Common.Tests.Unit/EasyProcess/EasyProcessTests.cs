...
internal sealed class EasyProcessTests
{
    ...
    public async Task When_starting_a_process_with_no_errors()
    {
        ...
        lines.Count(x => !x.IsError).ShouldBeGreaterThan(3);

        lines.Count(s => s.Line.Contains("Reply from 127.0.0.1:")).ShouldBe(3);
    ...
    public async Task When_starting_a_process_with_errors()
    {
        ...
        lines[0].Line.ShouldStartWith("Couldn't find a project to run.");
    ...
    public async Task When_starting_a_process_then_cancelling_execution_after_a_while()
    {
        ...
        lines.Count(x => x.IsError).ShouldBe(0);
        lines.Count(s => s.Line.Contains("Reply from 127.0.0.1:")).ShouldBeGreaterThan(1);
    ...
    public async Task When_starting_a_process_with_env_var_then_should_be_able_to_read_it()
    {
        ...
        proc1Lines.ShouldContain(x => x.Line == "Environment variable SOME_ENV not defined");
        ...
        proc2Lines.ShouldContain(x => x.Line == "SOME_ENV=some-value");
