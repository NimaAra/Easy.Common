...
internal sealed class RestClientTests
{
    ...
    public void When_sending_a_request_with_cancellation_and_completion_option()
    {
        ...
        var request = new HttpRequestMessage
        {
            ...
            RequestUri = new Uri("http://example.org/api/24")
        };
    ...
    public void When_sending_a_request_with_cancellation()
    {
        ...
        {
            Method = HttpMethod.Get,
            RequestUri = new Uri("http://example.org/api/25")
        };
    ...
    public void When_sending_a_request_then_cancelling_all_pending_requests()
    {
        ...
        {
            Method = HttpMethod.Get,
            RequestUri = new Uri("http://example.org/api/26")
        };
...
internal sealed class FakeResponseHandler : DelegatingHandler
{
    ...
    public void AddFakeResponse(Uri uri, HttpResponseMessage response) => _responses.Add(uri.OriginalString, response);

    protected override Task<HttpResponseMessage> SendAsync(
        HttpRequestMessage request, CancellationToken cToken) => Task.FromResult(
                _responses.TryGetValue(request.RequestUri?.OriginalString ?? string.Empty, out HttpResponseMessage response)
